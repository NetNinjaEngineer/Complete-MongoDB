...
db.users.updateOne(
  ...
  { $set: { "subscription_type.type": "hamboozo" } }
);
...
db.users.findOne({ _id: ObjectId("67c31bae1e062a24824d7953") })
  .subscription_type.renewal_date.year;
...
db.orders.insertMany([
  ...
  },
  {
    ...
    products: [ObjectId("673b5f9471bf64a76286bb23")],
  ...
  },
  {
    ...
    products: [ObjectId("673b5f9471bf64a76286bb2a")],
  ...
  },
  {
    ...
    products: [ObjectId("673b5f9471bf64a76286bb35")],
  ...
  },
  {
    ...
    products: [ObjectId("673b5f9471bf64a76286bb3c")],
  ...
  },
  {
    ...
    products: [ObjectId("673b5f9471bf64a76286bb43")],
  ...
  },
  {
    ...
    products: [ObjectId("673b5f9471bf64a76286bb4e")],
...
// OR operator
db.products.find({
  $or: [{ ratings: { $gt: 7 } }, { category: { $in: ["mobile"] } }],
});
db.products
  .find({ $or: [{ ratings: { $gt: 7 } }, { category: { $in: ["mobile"] } }] })
  .count(); // 22

//NOR operator
// match any products that does not match any of the specified conditions
db.products.find({
  $nor: [{ ratings: { $gt: 7 } }, { category: { $in: ["mobile"] } }],
});
db.products
  .find({ $nor: [{ ratings: { $gt: 7 } }, { category: { $in: ["mobile"] } }] })
  .count(); // 8

// AND operator
// get all products with ratings greater than or equal to 7 and price less than or equals to 500
db.products.find({
  $and: [{ ratings: { $gte: 7 } }, { price: { $lte: 500 } }],
});
db.products
  .find({ $and: [{ ratings: { $gte: 7 } }, { price: { $lte: 500 } }] })
  .count(); // 3
...
db.employees.updateOne(
  { name: "Alice Smith" },
  { $set: { hobbies: ["reading", "hiking", "music"] } }
);
...
db.employees.find({
  skills: { $elemMatch: { name: "designer", level: { $gt: 2 } } },
});
...
db.products.find(
  { ratings: { $gte: 8 } },
  { _id: 0, name: 1, category: { $slice: 2 } }
);
db.products.find(
  { ratings: { $gte: 8 } },
  { _id: 0, name: 1, category: { $slice: 1 } }
);

// show elements from index 1 and takes three elements from the category
db.products.find(
  { ratings: { $gte: 8 } },
  { _id: 0, name: 1, category: { $slice: [1, 3] } }
);

// Update operation in depth

db.getCollection("employees").find();

// Note ==> if the update value is the same mongodb not perform the update operation
db.getCollection("employees").updateOne(
  { _id: ObjectId("67dd2e64725d9550194d7944") },
  { $set: { isAdmin: true } }
);

db.getCollection("employees").findOne({
  _id: ObjectId("67dd2e64725d9550194d7944"),
});
...
db.getCollection("employees").insertOne({
  name: "Mohamed Ehab",
  age: 22,
  skills: [{ name: 'developer', level: 7 }, { name: 'producer', level: 5 }],
  hobbies: ['traveling', 'music', 'coding']
})


// get the developers employees and add isDeveloper field to them

db.getCollection("employees").find({ "skills.name": "developer" })
db.getCollection("employees").find({ hobbies: "coding" })


db.getCollection("employees").updateMany(
